...
%
% Two stage GMM procedure to obtain param estimates, standard errors, J
% statistic
%
% Inputs
% ------
...
%             for parameter vector theta. Typically a wrapper function
%             for a numerical optimization routine which uses theta_init
%             as starting point for numerical search
...
% twostage    Whether really to do two-stages or just stop at one stage
    ...
    est.stderrs0  = (Ghat0'*W0*Ghat0)\(Ghat0'*W0*Omegahat0*W0*Ghat0)*inv(Ghat0'*W0*Ghat0);
    ...
    if twostage
      ...
      est.stderrs1  = inv(Ghat1'*inv(Omegahat1)*Ghat1);
